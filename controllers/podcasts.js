...
export const createPodcast = async (req, res, next) => {
    try {
        ...
        await Promise.all(req.body.episodes.map(async (item) => {
            ...
            console.log("Attempting to save episode:", episode);
            try {
                const savedEpisode = await episode.save();
                console.log("Saved episode:", savedEpisode);
                episodeList.push(savedEpisode._id);
            } catch (saveErr) {
                console.error("Error saving episode:", saveErr);
                throw saveErr; // Rethrow the error to be caught by the outer catch block
            }
        }));
        ...
        console.error("Error in createPodcast:", err);
        next(err);
    }
};
